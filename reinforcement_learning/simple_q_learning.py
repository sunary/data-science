...
weight = [[-1.0] * 100 for _ in range(100)]
...
def q_learning(src=(7, 9), dest=(0, 0)):
                    ...
                    if i + s[0] in range(BOARD_SIZE) and j + s[1] in range(BOARD_SIZE):
                        ...
                        for s2 in sign4:
                            if i + s[0] + s2[0] in range(BOARD_SIZE) and j + s[1] + s2[1] in range(BOARD_SIZE) and \
                                        weight[(i + s[0]) * BOARD_SIZE + (j + s[1])][(i + s[0] + s2[0]) * BOARD_SIZE +
                                                (j + s[1] + s2[1])] >= 0:
                                cell_weights.append(weight[(i + s[0]) * BOARD_SIZE + (j + s[1])][
                                                        (i + s[0] + s2[0]) * BOARD_SIZE + (j + s[1] + s2[1])])

                        weight[i * BOARD_SIZE + j][(i + s[0]) * BOARD_SIZE + (j + s[1])] = \
                            max(weight[i * BOARD_SIZE + j][(i + s[0]) * BOARD_SIZE + (j + s[1])],
                                learning_rate * max(cell_weights))
    ...
    while True:
        ...
        for s in sign4:
            if i + s[0] in range(BOARD_SIZE) and j + s[1] in range(BOARD_SIZE):
                cell_weights.append(
                    (weight[i * BOARD_SIZE + j][(i + s[0]) * BOARD_SIZE + (j + s[1])], i + s[0], j + s[1]))
    ...
    for i in range(BOARD_SIZE ** 2):
        for j in range(BOARD_SIZE ** 2):
            if weight[i][j] > 0:
                print str(((i / BOARD_SIZE, i % BOARD_SIZE), (j / BOARD_SIZE, j % BOARD_SIZE))) + ': ' + str(
                    weight[i][j])
