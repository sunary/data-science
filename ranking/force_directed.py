...
class ForceDirected():
    ...
    def input(self, vertex, edge):
        ...
        for i in range(self.graph.num_vertex):
            self.graph.vertex[i].set_pos(
                [self.WIDTH / 3 * (random.random() - 0.5), self.HEIGHT / 3 * (random.random() - 0.5)])
        ...
        for i in range(num_loops):
            ...
            for v1 in range(self.graph.num_vertex):
                ...
                for v2 in range(self.graph.num_vertex):
                    if v1 != v2:
                        temp_vertex = Vertex()
                        temp_vertex.set_pos([self.graph.vertex[v1].pos[0] - self.graph.vertex[v2].pos[0],
                                             self.graph.vertex[v1].pos[1] - self.graph.vertex[v2].pos[1]])
            ...
            for e in range(self.graph.num_edge):
                if not self.graph.vertex[self.graph.edge[e][1]].visited:
                    temp_vertex = Vertex()
                    temp_vertex.set_pos([self.graph.vertex[self.graph.edge[e][0]].pos[0] -
                                         self.graph.vertex[self.graph.edge[e][1]].pos[0],
                                         self.graph.vertex[self.graph.edge[e][0]].pos[1] -
                                         self.graph.vertex[self.graph.edge[e][1]].pos[1]])
            ...
            for v in range(self.graph.num_vertex):
                ...
                self.graph.vertex[v].pos[0] += (self.graph.vertex[v].disp[0] / norm) * min(
                    math.fabs(self.graph.vertex[v].disp[0]), self.t)
                self.graph.vertex[v].pos[1] += (self.graph.vertex[v].disp[1] / norm) * min(
                    math.fabs(self.graph.vertex[v].disp[1]), self.t)
